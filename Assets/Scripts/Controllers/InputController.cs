...
namespace Controllers
{
    public sealed class InputController : IInputController
    {
        ...
        public event EventHandler<FieldCoords> MouseDoubleClick;
        ...
        public void CustomUpdate()
        {
            ...
            if (_buttonDownPosition != null)
                if ((Mathf.Abs(_previousMousePosition.x - Input.mousePosition.x) > DeltaRegistration
                     || Mathf.Abs(_previousMousePosition.y - Input.mousePosition.y) > DeltaRegistration)
                    && (Mathf.Abs(_buttonDownPosition.Value.x - Input.mousePosition.x) > DeltaRegistration
                        || Mathf.Abs(_buttonDownPosition.Value.y - Input.mousePosition.y) > DeltaRegistration))
                {
                    _isOneClick = false;
                    OnMouseMove(CoordinateConverter.CameraPositionToFieldCoords(_buttonDownPosition.Value) - CoordinateConverter.CameraPositionToFieldCoords(Input.mousePosition));
                }
        }

        private bool IsCorrectSwipe()
        {
            if (_buttonDownPosition == null) return false;
            var coordsStartSwipe = CoordinateConverter.CameraPositionToFieldCoords(_buttonDownPosition.Value);
            var coordsEndSwipe = CoordinateConverter.CameraPositionToFieldCoords(Input.mousePosition);
        ...
        private void OnMouseDoubleClick(FieldCoords coords)
        {
            MouseDoubleClick?.Invoke(this, coords);
        }
